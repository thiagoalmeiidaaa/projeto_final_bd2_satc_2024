    ...
    if ($operacao === 'delete') {
        ...
        if ((strlen($_POST['nome']) == 0) && ($operacao != 'delete')) {
            ...
            if ($operacao == 'insert') {
                ...
                $sql_verifica = "SELECT nome, email, senha FROM clientes WHERE id_cliente = ?";
                $stmt_verifica = $mysqli->prepare($sql_verifica);
                $stmt_verifica->bind_param("i", $id_cliente);
                $stmt_verifica->execute();
                $stmt_verifica->store_result();
                $stmt_verifica->bind_result($old_nome, $old_email, $old_senha);
                $stmt_verifica->fetch();
                ...
                if ($stmt_verifica->num_rows > 0) {
                    ...
                    if ($stmt_update) {
                        ...
                        $changes = '';
                        if ($old_nome != $nome) {
                            $changes .= "nome mudou de $old_nome para $nome; ";
                        }
                        if ($old_email != $email) {
                            $changes .= "email mudou de $old_email para $email; ";
                        }
                        if ($old_senha != $senha) {
                            $changes .= "senha mudou de $old_senha para $senha; ";
                        }
            
                        if ($changes != '') {
                            $sql_log = "INSERT INTO log_eventos (evento, data_hora) VALUES (?, NOW())";
                            $stmt_log = $mysqli->prepare($sql_log);
                            $evento = "Update no cliente! - $changes";
                            $stmt_log->bind_param("s", $evento);
                            $stmt_log->execute();
                            $stmt_log->close();
                        }
